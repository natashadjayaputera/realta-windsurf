...
1.1 For each `chunks_cs/{ProgramName}/{SubProgramName}CLS/**_R_BatchProcess.cs` found, save all the {SubProgramName} in a `chunks_cs/{ProgramName}/viewmodel_with_batch_list_buffer.txt` file. 
1.2 For each line in `chunks_cs/{ProgramName}/viewmodel_with_batch_list_buffer.txt` file:
- Read `chunks_cs/{ProgramName}/{SubProgramName}CLS/**_R_BatchProcess.cs` and search for `R_NetCoreUtility.R_DeserializeObjectFromByte<List<T>>` and assign T as `{BatchDisplayListDTO}`.
- Read `chunks_cs/{ProgramName}/{SubProgramName}CLS/**_R_BatchProcess.cs` and get the LAST occurance of `RSP_WriteUploadProcessStatus` stored procedure and extract all 6 of the stored procedure arguments, and assign the 4th argument as `{BatchSteps}`.
...
2.4 If not approved, ask for changes and repeat step 2.2.