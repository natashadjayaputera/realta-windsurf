---
name: 04-create-additional-files-in-common-project
...
1.1 Loop through each `{SubProgramName}` folder inside `chunks_cs/{ProgramName}`.
1.2 For each `{SubProgramName}`, execute Phase 2 – Phase 3.

## Phase 2: Extract `ClassDeclaration.txt` file
2.1 From `chunks_cs/{ProgramName}/{SubProgramName}`, read `ClassDeclaration.txt`.
...
3.1 From `chunks_cs/{ProgramName}/{SubProgramName}`, read `functions.txt`.
