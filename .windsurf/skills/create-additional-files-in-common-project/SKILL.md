...
1.1 Decide if it is inheriting R_BusinessObject
1.2 Decide if it is implementing R_IBatchProcess

## Phase 2: Create Interface for {SubProgramName}
2.1 Read common_interface_template as a template and read `functions.txt` file and create `I{SubProgramName}.cs` in `{ProgramName}Common` Project root folder.
