# Create Program Resources Script
# Generates Resources project for program-level resources
# Reads resource files from partials\{ProgramName}\resources\*.txt
...
function New-ProgramResourcesProject {
    param(
        [string]$OutputPath,
        [string]$ProgramName,
    ...
    # Path to search for program resources folder
    try {
        $searchPath = Join-Path $gitRoot "partials\$ProgramName\resources"
        if (-not (Test-Path $searchPath)) {
            Write-Warning "Program resources directory not found: $searchPath"
            return
        }
        
        $resourceFiles = Get-ChildItem -Path $searchPath -Filter "*.txt" -ErrorAction SilentlyContinue
    }
    catch {
        Write-Error "Error searching for program resource files: $($_.Exception.Message)"
        return
    }
    
    Write-Host "Creating Program Resources project"
    ...
    # Get all .txt resource files from program resources folder
    if ($resourceFiles.Count -eq 0) {
        Write-Warning "No resource files found in program resources directory: $searchPath"
        return
    }
    ...
    foreach ($resourceFile in $resourceFiles) {
        ...
        # Read resource entries (all lines are resource entries for program resources)
        try {
            $lines = Get-Content -Path $resourceFile.FullName -ErrorAction Stop
            Write-Host "Processing all $($lines.Count) resource entries"
        }
        catch {
        ...
        # Use base output path for program resources
        $projectResourcesPath = Join-Path $resourcesProjectPath "$($projectName)Resources"
...
"@
        ...
        # Generate default resx content (all lines are resource entries)
        $defaultResxContent = Generate-ResxContent -ResourceEntries $lines -DefaultLanguage $DefaultLanguage -AdditionalLanguages $AdditionalLanguages
...
# Main execution
try {
    New-ProgramResourcesProject -OutputPath $OutputPath -ProgramName $ProgramName -DefaultLanguage $DefaultLanguage -AdditionalLanguages $additional
}
catch {
    Write-Error "Error creating Program Resources project: $($_.Exception.Message)"
    exit 1
}
