...
1.2 From `chunks_cs/{ProgramName}/{SubProgramName}`, edit `ClassDeclaration.txt` to follow new standard.
...
2.2 From `chunks_cs/{ProgramName}/{SubProgramName}`, edit each `XXXX_FunctionName.cs` to follow new standard.
...
3.1 Execute `{ROOT}/.windsurf/scripts/fix-indentation.ps1`:
- ProgramName = {ProgramName}
- RootPath = {ROOT}
Example:
`powershell -ExecutionPolicy Bypass -File "{ROOT}/.windsurf/scripts/fix-indentation.ps1" -ProgramName "FAI00110" -RootPath "D:\_Work\AI\realta-windsurf"`