---
name: report_controller_template
description: "Template for Report Controller for Service Project report endpoints"
---
...
namespace {ProgramName}Service
{
    public class {SubProgramName}ReportController : R_ReportControllerBase
    {
        ...
        private {SubProgramName}ReportParam _Parameter;
        private readonly ActivitySource _activitySource;

        public {SubProgramName}ReportController(ILogger<{SubProgramName}ReportController> logger)
        {
            Logger{ProgramName}.R_InitializeLogger(logger);
            _logger = Logger{ProgramName}.R_GetInstanceLogger();
            _activitySource = {ProgramName}Activity.R_InitializeAndGetActivitySource(nameof({SubProgramName}ReportController));
        ...
        private void _ReportCls_R_InstantiateMainReportWithFileName(ref string pcfiletemplate)
        {
            ...
            pcfiletemplate = Path.Combine("Reports", "{SubProgramName}Report.frx");
        }
        ...
        [HttpPost]
        public R_DownloadFileResultDTO {SubProgramName}Post({SubProgramName}ReportParam poParam)
        {
            using var loActivity = _activitySource.StartActivity(nameof({SubProgramName}Post));
            _logger.LogInfo("Start - Post {SubProgramName} Report");
            R_Exception loException = new();
            {SubProgramName}ReportLogKeyDTO loCache = null;
            ...
            try
            {
                ...
                loCache = new {SubProgramName}ReportLogKeyDTO
                {
                    poParam = poParam,
                ...
                _logger.LogInfo("Set GUID Param - Post {SubProgramName} Report");
        ...
        public FileStreamResult {SubProgramName}Get(string pcGuid)
        {
            using var loActivity = _activitySource.StartActivity(nameof({SubProgramName}Get));
            _logger.LogInfo("Start - Get {SubProgramName} Report");
            R_Exception loException = new();
            FileStreamResult loRtn = null;
            {SubProgramName}ReportLogKeyDTO loResultGUID = null;
            try
            {
                // Get cached parameter
                loResultGUID = R_NetCoreUtility.R_DeserializeObjectFromByte<{SubProgramName}ReportLogKeyDTO>(
            ...
            loException.ThrowExceptionIfErrors();
            _logger.LogInfo("End - Get {SubProgramName} Report");
            return loRtn;
        }

        private {SubProgramName}ReportWithBaseHeaderDTO GenerateData({SubProgramName}ReportParam poParam)
        {
            using var loActivity = _activitySource.StartActivity(nameof(GenerateData));
            var loEx = new R_Exception();
            var loRtn = new {SubProgramName}ReportWithBaseHeaderDTO();
...
# {SubProgramName}ReportLogKeyDTO Pattern
```csharp
using R_BackEnd;
using R_CommonFrontBackAPI.Log;
using {ProgramName}Common.DTOs;
...
{
    public class {SubProgramName}ReportLogKeyDTO
    {
        public {SubProgramName}ReportParam poParam { get; set; } = new();
...
# {SubProgramName}ReportParam
```csharp
namespace {ProgramName}Common.DTOs
{
    public class {SubProgramName}ReportParam
    {
        public bool LIS_PRINT { get; set; }
        public string CREPORT_FILETYPE { get; set; } = "PDF";
        public string CREPORT_FILENAME { get; set; } = "{SubProgramName}Report";
    }
}
```

# {SubProgramName}ReportWithBaseHeaderDTO
```csharp
using BaseHeaderReportCOMMON;
...
namespace {ProgramName}Common.DTOs
{
    public class {SubProgramName}ReportWithBaseHeaderDTO : BaseHeaderResult
    {
        //public List<{SubProgramName}ReportDataResultDTO> Data { get; set; } = new();
    }
};
```
...
4. **Endpoints**:
   - `[HttpPost] {SubProgramName}Post`: Cache params, return GUID
   - `[HttpGet, AllowAnonymous] {SubProgramName}Get`: Generate and stream report
5. **GenerateData**: Implement report-specific data retrieval and DTO building

## DTOs Required

- `{SubProgramName}ReportParam` - Report parameters (placed in Common Project)
- `{SubProgramName}ReportLogKeyDTO` - Cache structure with param, log key, global vars (placed in Service Project)
- `{SubProgramName}ReportWithBaseHeaderDTO` - Complete report data structure, must inherit `BaseHeaderResult` (placed in Common Project)
