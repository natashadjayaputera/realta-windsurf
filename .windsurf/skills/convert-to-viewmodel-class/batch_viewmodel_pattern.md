---
name: batch_viewmodel_pattern
description: "Batch data manipulation ViewModel implementation pattern in Model Project"
---
...
- **VMM001**: "Process Complete and success with GUID {0}"
- **VMM002**: "Process Complete but fail with GUID {0}"
- **VMM003**: "Process Error with GUID {0}"
- **VMM004**: "Process Progress {0} with status {1}"

## Checklist
- [ ] `liStep` cannot be 0, should follow the implementation in VB.NET (.NET Framework 4) implementation in Front Project
- [ ] If `liStep` is not specified and `_BatchProcessAsync` in `{ProgramName}BatchCls` uses SP and Temp Table, `liStep` must match SP step counts
- [ ] If `liStep` is not specified and `_BatchProcessAsync` in `{ProgramName}BatchCls` uses foreach loop with each data Insertion, `liStep` is the total count of Data
