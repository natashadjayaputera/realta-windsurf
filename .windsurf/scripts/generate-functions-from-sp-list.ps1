...
function Generate-OtherFunction {
    ...
    $parameterPart = if ($hasParameterDTO) { "$($SubProgramName)$($StoredProcedureName)ParameterDTO poParam, DbConnection? poConnection = null" } else { "DbConnection? poConnection = null" }
...
foreach ($line in $spListContent) {
    ...
    if ($functionContent) {
        ...
        $functionSignature = switch ($category) {
            ...
            default { "public async Task $storedProcedureName(DbConnection? poConnection = null) //CATEGORY: $category" }
        }
